...
from cais_stability.experiments import CampaignConfig, EpisodeConfig, aggregate_records, run_campaign, run_episode
...
def test_episode_is_reproducible_under_seed() -> None:
    ...
    first = run_episode(DomainName.MARITIME, DynamicCAISPolicy(), disturbance, config)
    second = run_episode(DomainName.MARITIME, DynamicCAISPolicy(), disturbance, config)
    assert first == second


def test_campaign_cardinality() -> None:
    records = run_campaign(CampaignConfig(domains=(DomainName.MARITIME,), intensities=(0.25, 0.75), seeds=(0, 1), horizon=20, disturbance_onset=4, disturbance_duration=3))
    assert len(records) == 16


def test_campaign_metrics_are_bounded() -> None:
    records = run_campaign(CampaignConfig(intensities=(0.5,), seeds=(0,), horizon=25, disturbance_onset=4, disturbance_duration=3))
...
def test_aggregation_produces_one_row_per_method() -> None:
    records = run_campaign(CampaignConfig(domains=(DomainName.SMART_GRID,), intensities=(0.5,), seeds=(0, 1), horizon=20, disturbance_onset=4, disturbance_duration=3))
