...
def main() -> None:
    ...
    parser.add_argument(
        "--input",
        type=Path,
        default=Path("results/raw/article_results.csv"),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results/aggregated/article_summary.csv"),
    )
        ...
        for row in csv.DictReader(handle):
            records.append(
                EpisodeRecord(
                    ...
                    hard_violation_rate=float(row["hard_violation_rate"]),
                    recovery_success=row["recovery_success"] == "True",
                    recovery_time=_optional_int(row["recovery_time"]),
                    autonomy_retention=float(row["autonomy_retention"]),
                    intervention_rate=float(row["intervention_rate"]),
                    fallback_rate=float(row["fallback_rate"]),
                    mean_task_utility=float(row["mean_task_utility"]),
