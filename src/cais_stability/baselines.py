...
class UngovernedPolicy:
    ...
    def decide(self, state: SystemState, proposed_action: float) -> GovernanceDecision:
        ...
        return GovernanceDecision(
            ...
            proposed_action,
            GovernanceMode.AUTO,
            abs(action - proposed_action) > 1e-12,
        )
...
class DynamicCAISPolicy:
    ...
    def __init__(
        ...
        *,
        options: DynamicPolicyOptions | None = None,
        name: str | None = None,
    ) -> None:
    ...
    def _constrained_decision(
        self,
        state: SystemState,
        proposed_action: float,
    ) -> GovernanceDecision:
        risk = self.governance.effective_risk(state)
    ...
    def decide(self, state: SystemState, proposed_action: float) -> GovernanceDecision:
        if self.options.use_uncertainty:
            control_state = state
        else:
            control_state = SystemState(state.risk, state.degradation, 0.0)
        ...
        if decision.mode is GovernanceMode.FALLBACK and not self.options.enable_fallback:
            if self.options.enable_recovery:
                decision = GovernanceDecision(
                    self.governance.config.recovery_action,
                    proposed_action,
                    GovernanceMode.RECOVER,
                    True,
                )
            else:
                decision = self._constrained_decision(control_state, proposed_action)
...
def ablation_policies() -> tuple[GovernancePolicy, ...]:
    return (
        DynamicCAISPolicy(),
        DynamicCAISPolicy(
            options=DynamicPolicyOptions(enable_recovery=False),
            name="dynamic_no_recovery",
        ),
        DynamicCAISPolicy(
            options=DynamicPolicyOptions(use_uncertainty=False),
            name="dynamic_no_uncertainty",
        ),
        DynamicCAISPolicy(
            options=DynamicPolicyOptions(use_hysteresis=False),
            name="dynamic_no_hysteresis",
        ),
        DynamicCAISPolicy(
            options=DynamicPolicyOptions(enable_fallback=False),
            name="dynamic_no_fallback",
        ),
    )
